...
public class HotelSimulatorTest {
     ...
     *
     * We create a Guest with ID 42 and check that getId() gives back 42.
    ...
    void guest_isNotCheckingOutByDefault() {
        ...
        // ASSERT (no ACT needed – we're checking the default state)
     ...
     *
     * Calls setCheckingOut(true) and then verifies isCheckingOut() returns true.
    ...
    void subTile_isNotWalkableWhenOccupiedByPerson() {
        ...
        // ACT – place the guest on the sub-tile
        subTile.setPerson(guest);
    ...
    void subTile_globalCoordinatesAreCorrect() {
        ...
        // ASSERT
        assertEquals(5, subTile.getGlobalX()); // 2*2 + 1 = 5
        assertEquals(6, subTile.getGlobalY()); // 3*2 + 0 = 6
    }
    ...
    private Tile[] buildLinearCorridor() {
        // ARRANGE – three walkable tiles in a row
     ...
     *
     * We use the leftmost and rightmost sub-tile of the same tile.
     * assertFalse(path.isEmpty()) checks that A* returned at least one step.
     */
    @Test
    ...
    /**
     * Test: a newly created Area should not be claimed by any guest yet.
     *
    ...
    /**
     * Test: we should be able to claim an Area.
     *
     * setIsClaimed(true) marks the room as occupied.
     */
    @Test
